...
transitions:{},helpers:{},/*
		 *	Static methods
		 */
open:function(group,opts){return group&&($.isPlainObject(opts)||(opts={}),!1!==F.close(!0))?(// Normalize group
$.isArray(group)||(group=isQuery(group)?$(group).get():[group]),// Recheck if the type of each element is `object` and set content type (image, ajax, etc)
$.each(group,function(i,element){var href,title,content,type,rez,hrefParts,selector,obj={};"object"===$.type(element)&&(// Check if is DOM element
...
"ajax"===type&&(hrefParts=href.split(/\s+/,2),href=hrefParts.shift(),selector=hrefParts.shift())),content||("inline"===type?href?content=$(isString(href)?href.replace(/.*(?=#[^\s]+$)/,""):href):obj.isDom&&(content=element):"html"===type?content=href:type||href||!obj.isDom||(type="inline",content=element)),$.extend(obj,{href:href,type:type,content:content,title:title,selector:selector}),group[i]=obj}),// Extend the defaults
F.opts=$.extend(!0,{},F.defaults,opts),// All options are merged recursive except keys
opts.keys!==undefined&&(F.opts.keys=opts.keys?$.extend({},F.defaults.keys,opts.keys):!1),F.group=group,F._start(F.opts.index)):void 0},// Cancel image loading or abort ajax request
...
D.bind("keydown.loading",function(e){27===(e.which||e.keyCode)&&(e.preventDefault(),F.cancel())}),F.defaults.fixed||(viewport=F.getViewport(),el.css({position:"absolute",top:.5*viewport.h+viewport.y,left:.5*viewport.w+viewport.x}))},getViewport:function(){var locked=F.current&&F.current.locked||!1,rez={x:W.scrollLeft(),y:W.scrollTop()};return locked?(rez.w=locked[0].clientWidth,rez.h=locked[0].clientHeight):(// See http://bugs.jquery.com/ticket/6724
rez.w=isTouch&&window.innerWidth?window.innerWidth:W.width(),rez.h=isTouch&&window.innerHeight?window.innerHeight:W.height()),rez},// Unbind the keyboard / clicking actions
...
F.coming=coming,!1===F.trigger("beforeLoad"))return F.coming=null,void 0;if(type=coming.type,href=coming.href,!type)//If we can not determine content type then drop silently or display next/prev item if looping through gallery
...
w1=$(window).width(),H.addClass("fancybox-lock-test"),w2=$(window).width(),H.removeClass("fancybox-lock-test"),$("<style type='text/css'>.fancybox-margin{margin-right:"+(w2-w1)+"px;}</style>").appendTo("head")})}(window,document,jQuery),function($,window,document){$(document).ready(function(){function loadTile(tile){var imgElement=tile.find("img"),attr=imgElement.attr("data-src");"undefined"!=typeof attr&&attr!==!1&&imgElement.attr("src",attr),setTimeout(function(){tile.removeClass("hidden-tile").addClass("animated")},20)}function hideTile(tile){tile.addClass("hidden-tile").removeClass("animated");var imgElement=tile.find("img"),attr=imgElement.attr("src");"undefined"!=typeof attr&&attr!==!1&&(imgElement.attr("data-src",attr),imgElement.attr("src",attr))}function displayHints(w){w.scrollTop()>500?$(".back-to-top").fadeIn(duration):$(".back-to-top").fadeOut(duration)}function twoColumnsBehaviour(){$(".view-large").each(function(){var item=$(this);item.off("click"),item.on("click",function(e){e.preventDefault(),$.fancybox.open({href:item.attr("href"),closeBtn:!1,closeClick:!0})})}),$(window).scroll(function(){displayHints($(this)),clearTimeout($.data(this,"scrollTimer")),$.data(this,"scrollTimer",setTimeout(function(){if($(window).scrollTop()+$(window).height()>$(document).height()-500){var yearBox=$(".year-box .hidden-tile").first().closest(".year-box");yearBox.fadeIn(function(){loadTile(yearBox.find(".right-row .hidden-tile").first()),loadTile(yearBox.find(".left-row .hidden-tile").first());for(var difference=yearBox.find(".left-row").height()-yearBox.find(".right-row").height(),smthToLoad=!0;Math.abs(difference)>300&&yearBox.find(".hidden-tile").length>0&&smthToLoad;)difference>0?(yearBox.find(".right-row .hidden-tile").first().removeClass("hidden-tile").addClass("animated"),0===yearBox.find(".right-row .hidden-tile").length&&(smthToLoad=!1)):(yearBox.find(".left-row .hidden-tile").first().removeClass("hidden-tile").addClass("animated"),0===yearBox.find(".left-row .hidden-tile").length&&(smthToLoad=!1)),difference=yearBox.find(".left-row").height()-yearBox.find(".right-row").height()})}},250))})}function oneColumnBehaviour(){$(".right-row .info-tile").not(".hidden-tile").each(function(){hideTile($(this))}),$(".view-large").off("click"),$(".view-large").on("click",function(e){e.preventDefault()}),$(window).scroll(function(){displayHints($(this)),clearTimeout($.data(this,"scrollTimer")),$.data(this,"scrollTimer",setTimeout(function(){if($(window).scrollTop()+$(window).height()>$(document).height()-600){var yearBox=$(".year-box .hidden-tile").first().closest(".year-box");yearBox.fadeIn(function(){loadTile(yearBox.find(".hidden-tile").first())})}},250))})}var duration=500;$(".back-to-top").on("click",function(event){event.preventDefault(),$("html, body").animate({scrollTop:0},duration)});try{var mq=window.matchMedia("(min-width: 480px)");mq.matches?twoColumnsBehaviour(duration):oneColumnBehaviour(duration),mq.addListener(function(e){e.matches?twoColumnsBehaviour(duration):oneColumnBehaviour(duration)})}catch(err){oneColumnBehaviour(duration)}})}(jQuery,window,document);